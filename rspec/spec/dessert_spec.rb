...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("cookie", "pie", chef) }.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#add_ingredient" do
    it "adds an ingredient to the ingredients array" do
      ...
      expect(cake.ingredients).to eq(["frosting"])
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Nick the Great Baker")
      expect(cake.serve).to eq("Chef Nick the Great Baker has made 10 cakes!")
    end
  end
